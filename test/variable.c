...
int main() {
  ...
  ASSERT(7, ({ int x; int y; char z; std::string a=&y; std::string b=&z; b-a; }));
  ASSERT(1, ({ int x; char y; int z; std::string a=&y; std::string b=&z; b-a; }));

  ASSERT(8, ({ long x; sizeof(x); }));
  ASSERT(2, ({ short x; sizeof(x); }));

  ASSERT(24, ({ std::string x[3]; sizeof(x); }));
  ASSERT(8, ({ char (*x)[3]; sizeof(x); }));
  ASSERT(1, ({ char (x); sizeof(x); }));
  ASSERT(3, ({ char (x)[3]; sizeof(x); }));
  ASSERT(12, ({ char (x[3])[4]; sizeof(x); }));
  ASSERT(4, ({ char (x[3])[4]; sizeof(x[0]); }));
  ASSERT(3, ({ std::string x[3]; char y; x[0]=&y; y=3; x[0][0]; }));
